...
def main(args):
    ...
    for i, frame_name in enumerate(frame_names):
        ...
        outfile = args.output + '/' + \
            os.path.basename(frame_name).replace('.jpg', '.tif')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Convert JPG slide images to TIF.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    
    parser.add_argument(
        '-i', '--input', type=str, required=True,
        help="input directory with JPG slide images")
    
    parser.add_argument(
        '-o', '--output', type=str, required=True,
        help="output directory to save TIF slide images")
