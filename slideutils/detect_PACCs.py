...
def segment_frame(frame, params):
    ...
    event_mask = measure.label(target_mask)
    ...
    if params["mask_path"] is not None:
        ...
        frame.writeMask(
            params["mask_path"],
            name_format="Tile%06d_nucleus.tif",
            in_mask=nucleus_mask,
        )

    # Calculating basic features
    features = frame.calc_basic_features()
    nucleus_features = frame.calc_basic_features(in_mask=nucleus_mask)
    renamed_cols = {cn: "nuc_" + cn for cn in nucleus_features.columns[2:]}
    nucleus_features = nucleus_features.rename(columns=renamed_cols)
    features = features.merge(nucleus_features, on=("frame_id", "cell_id"))
...
def process_frames(args):
    ...
    params = {
        ...
        "nucleus_ch": args.nucleus_channel,
    ...
    all_features = pd.concat(
        [out["features"] for out in data], ignore_index=True
    )
    ...
    if len(args.filter) != 0:
        logger.info("Filtering events...")
        all_features = utils.filter_events(
            all_features, args.filter, args.verbose
        )
        logger.info("Finished filtering events.")
    all_images = (
        all_images[list(all_features.index)]
        if all_images is not None
        else None
    )
    all_masks = (
        all_masks[list(all_features.index)] if all_masks is not None else None
    )
