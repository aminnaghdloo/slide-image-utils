...
def main():
	...
	# Load inputs
	if len(sys.argv) != 3:
		print("Usage: python calc_min_dist.py <query.txt> <reference.txt>")
		sys.exit(-1)
	...
	for index, row in df_q.iterrows():
		...
		print(f"processed {index} / {len(df_q)} against {len(df_r)} references")

	dist_df = pd.DataFrame(dist_dict)
	out_df = pd.concat([df_q.iloc[:, :6], dist_df], axis=1)
	out_df.to_csv(f"{query_file_name[:-4]}_min_dist.txt", sep='\t', index=False)
