...
def extract_event_crops(args):
    ...
    all_images = np.zeros(
        (len(df), width, width, len(channels)), dtype="uint16"
    )
    if mask_flag:
        all_masks = np.zeros((len(df), width, width, 1), dtype="uint16")
    ...
    for frame_id, event_data in groups:
        ...
        logger.info(
            f"{len(event_data)} images extracted from frame {frame_id}"
        )
...
def main():
    ...
    parser.add_argument(
        "-m", "--mask", type=str, default=None, help="path to mask directory"
    )
