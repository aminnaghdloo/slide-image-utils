...
def filter_events(features, filters, verbosity):
    ...
    if features is None:
        # logger.warning("There is no frame event filter!")
        print("There is no frame event filter!")
        return None
    ...
    if sum(sel["index"].astype(int)) == 0:
        # logger.warning("Frame events did not pass the filter(s)!")
        print("Frame events did not pass the filter(s)!")
        return None

    features = features[sel["index"]]
    #logger.info(f"Filtered {n} events down to {len(features)} events")
    print(f"Filtered {n} events down to {len(features)} events")
...
def sort_events(features, sorts, verbosity):
    ...
    if features is None:
        logger.warning("Nothing left to sort!")
        return None
...
def channels2montage(images, b_index, g_index, r_index, order_index):
    ...
    if order_index:
        gray = np.concatenate(
            [images[:, :, :, k] for k in order_index], axis=2
        )
