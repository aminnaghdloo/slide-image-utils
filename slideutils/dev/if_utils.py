...
class Frame:
    ...
    def readNucleusMask(self):
        ...
        if os.path.exists(nucleusmask_path):
            ...
            print(
                f"Generating nucleus mask -> {self.slide_id} :"
                f" {self.frame_id}"
            )
    ...
    def readCellMask(self):
        ...
        if os.path.exists(cellmask_path):
            ...
            print(
                f"Generating cell mask -> {self.slide_id} :"
                f" {self.frame_id}"
            )
    ...
    def extractCellProps(self):
        ...
        names.extend(
            [f"{ch}_mean_intensity" for ch in list(channels.keys())[1:]]
        )
    ...
    def extractGranularity(self, n=10, channel=0, use_nucleus_mask=True):
        ...
        if use_nucleus_mask:
            df = measure.regionprops(
                self.nucleusmask, self.image[:, :, channel]
            )
        else:
        ...
        for i in range(len(df)):
            ...
            image = exposure.rescale_intensity(
                test_image,
                in_range=(test_image.min(), test_image.max()),
                out_range=(0, 255),
            )
            ...
            for j in range(n):
                ...
                ero_mask[mask] = ero[mask]
                ero = morphology.erosion(ero_mask, footprint=footprint)
                rec = morphology.reconstruction(
        ...
        out = pd.DataFrame(
            all_features,
            columns=[f"{prefix}_gran_{i}" for i in range(1, n + 1)],
        )
        return out
...
def getFrameImage(slide_id, frame_id):
    ...
    # JPG images
        ...
        image_names = [
            image_names[i].replace(img_suf1, img_suf2)
            for i in range(len(image_names))
        ]
        meta_names = [
            image_names[i].replace(img_suf2, img_meta)
            for i in range(len(image_names))
        ]
        ...
        out_image = np.stack(
            [
                out_image[:, :, i] * (
					meta_vals["maxval"][i] - meta_vals["minval"][i]
				) + meta_vals["minval"][i]
...
def segmentCell(image, seed, file_name=None):
    ...
    for i in range(image.shape[2]):
        ...
        image[:, :, i] = cv2.normalize(image[:, :, i], 0, 255, cv2.NORM_MINMAX)
...
def segment40xCell(image, seed, file_name=None):
    ...
    for i in range(image.shape[2]):
        ...
        image[:, :, i] = cv2.normalize(
            image[:, :, i], 0, 65535, cv2.NORM_MINMAX
        )
...
def getCellCoords(input_file, output_file=None):
    ...
    for slide_id, frame_id in groups.indices:
        ...
        print(
            f"{frame.count} cell coordinates extracted from slide_id:"
...
def getCellFeatures(input_file, output_file=None):
    ...
    for slide_id, frame_id in groups.indices:
        ...
        output_df.append(temp)
        print(
            f"{frame.count} cell features extracted from slide_id:"
            f"{slide_id}, frame_id:{frame_id}"
        )
    output_df = pd.concat(output_df)
    if output_file is not None:
...
def getGranularity(
    input_file, output_file=None, n=10, channel=0, use_nucleus_mask=False
):
    input_df = pd.read_table(input_file)
    input_df.frame_id = input_df.frame_id.astype("int")
    groups = input_df.groupby(["slide_id", "frame_id"])
    ...
    for slide_id, frame_id in groups.indices:
        frame = Frame(slide_id, frame_id)
        temp = frame.extractGranularity(
            n=n, channel=channel, use_nucleus_mask=use_nucleus_mask
        )
        output_df.append(temp)
        print(
            f"{frame.count} cell granularity extracted from slide_id:"
            f"{slide_id}, frame_id:{frame_id}"
        )
    output_df = pd.concat(output_df)
    if output_file is not None:
...
def getCellIdFromCoords(
    input_file, output_file=None, colname="new_cell_id", verbose=True
):
    """
...
def getCellImages(input_file, width, mask_flag=True):
    ...
    images = np.zeros(
        (cell_info.shape[0], width, width, len(channels)), dtype="uint16"
    )
    ...
    for slide_id, frame_id in groups.indices:
        ...
        images[index.tolist(), :, :, :] = cropCells(
            frame, cell_info.loc[index.tolist(), ["x", "y"]], width, mask_flag
        )
        print(
            f"{len(index)} cell images extracted from slide_id:{slide_id}"
            f", frame_id:{frame_id}"
        )
    return images
...
def cropCells(frame, xy_data, width, mask_flag):
    ...
    image = cv2.copyMakeBorder(
        frame.image, edge, edge, edge, edge, cv2.BORDER_CONSTANT, pad_color
    )
    indices = xy_data.index.tolist()
    out = np.zeros(
        (xy_data.shape[0], width, width, len(channels)), dtype="uint16"
    )

    if mask_flag:
        mask = cv2.copyMakeBorder(
            frame.cellmask, edge, edge, edge, edge, cv2.BORDER_CONSTANT, 0
        )
    ...
    for slide_id, frame_id in groups.indices:
        ...
        images.append(
            extractEventCrops(
                image,
                mask,
                cell_info.loc[index.tolist(), ["x", "y"]],
                width,
                mask_flag,
            )
        )
...
def extractEventCrops(image, mask, xy_data, width, mask_flag=0):
    ...
    mask = cv2.copyMakeBorder(
        mask, edge, edge, edge, edge, cv2.BORDER_CONSTANT, 0
    )
    mask = mask[..., np.newaxis]

    pad_color = np.zeros(image.shape[-1])
    image = cv2.copyMakeBorder(
        image, edge, edge, edge, edge, cv2.BORDER_CONSTANT, pad_color
    )
...
def get40xFrameImage(slide_id, frame_id):
    ...
    image_names = [
        f"{slide_path}/{frame_id}-{ch}.tif" for ch in channels.keys()
    ]
    out_image = np.stack(
        [tff.imread(image_names[i]) for i in range(len(channels))], axis=2
    )
    return out_image * 16
...
def getCoords(mask):
    ...
    props = measure.regionprops_table(
        mask, mask, separator="_", properties=["label", "centroid"]
    )
    props = pd.DataFrame(props)
    props.set_axis(["cell_id", "x", "y"], axis=1, inplace=True)
    return props
