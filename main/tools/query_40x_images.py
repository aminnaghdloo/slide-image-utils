...
# global variables:
path_template = "/mnt/Y/DZ/{}/ClientBin/40X"# template for path reimaging data
...
def main():
    ...
    # collect event data
    event_data = {'slide_id':[], 'frame_id':[], 'cell_id':[], 'x':[], 'y':[]}
    ...
    for slide in slides:
        ...
        for image_name in image_names:
            ...
            image = [
                tff.imread(ch_path)
                for ch_path in ch_paths if os.path.exists(ch_path)
            ]
    ...
    df.to_csv(f"event_data_{os.path.basename(file_name)}", index=False, sep='\t')
