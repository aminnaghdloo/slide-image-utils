...
def get_noise_qc(frames, threshold):
    ...
    df = pd.DataFrame({'frame_id':frame_ids, 'spot_count':spot_count,
                       'area_mean':spot_area_mean, 'area_dev':spot_area_sdev})
...
def main(args):
    ...
    for i in range(args.nframes):
        ...
        paths = utils.generate_tile_paths(
            path=args.input, frame_id=frame_id, starts=[args.start],
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-c', '--channel', type=str, default='TRITC',
    ...
    parser.add_argument(
        '-t', '--threshold', type=int, default=65000,
